...
# 3. PROFESYONEL PLAKA TANIMA (Plate Recognizer)
...
def handle_photo(message):
    ...
    if 'results' in data and len(data['results']) > 0:
        ...
        bot.reply_to(message, f"✅ Tespit: *{plaka}*\nKayıt için 'Giriş {plaka}' yaz.", parse_mode="Markdown")
    else:
        bot.reply_to(message, "❌ Plaka bulunamadı. Manuel giriş yap.")
...
def islem(message):
    ...
    if metin.startswith("GİRİŞ"):
        plaka = metin.replace("GİRİŞ", "").strip()
        ...
        if plaka in data["araclar"]:
            ...
            verileri_kaydet(data)
            bot.reply_to(message, f"📤 {plaka} çıkış. Ödeme: {ucret} DENAR")
